 ...
 * If the user is adding initial liquidity, user decides the ether and CD tokens he wants to add to the exchange. If he is adding the liquidity after the initial liquidity has already been added
 * the we calculate the Crypto Dev tokens he can add, given the Eth he wants to add by keepiong the ratios constant
 */
...
export const calaculateCD = async (
  ...
  // `_addEther` is a string we need to covert it to a Bignumber before we can do our calculations
  // We do that using the `parseEther` function from `ethers.js`
  const _addEtherAmountWei = utils.parseEther(_addEther);

  // Ratio needs to be maintained when we add addLiquidity.
  ...
  // So by maths we get (amount of Crypto Dev tokens to be added) = (Eth that would be added * Crypto Dev token balance) / (Eth reserve in the contract)
